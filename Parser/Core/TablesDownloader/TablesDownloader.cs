...
internal class TablesDownloader : ITablesDownloader
{
    ...
    public async Task DownloadTables(Dictionary<string, List<string>> linksInfo, CancellationToken token)
    {
        ...
        foreach (KeyValuePair<string, List<string>> kvp in linksInfo)
        {
            ...
            using (var stream = await httpClient.GetStreamAsync(kvp.Key, token))
            {
